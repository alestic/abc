...
abc uses a generative AI large language model to generate bash or zsh commands based on an English description provided by the user. The program takes the entire command line (excluding options) as the input description and returns command(s) that aim to accomplish the described task.
...
1. Interactive mode (recommended):
   ...
   In this example, you'd see `git status -s` in an editable prompt. You can modify it if needed, then press Enter to execute.
